...
from src.training.model_saint_plus import SAINTPlus
...
def get_data(df, max_length, split_index=0, dataset_name='elemmath_2021'):
    ...
    """
    item_ids, skill_ids, labels, indices, elapsed_time, lagged_time = [], [], [], [], [], []
    for idx, u_df in df.groupby("user_id"):
        ...
        elapsed_time.append(torch.tensor(u_df["resp_time_cat"].values, dtype=torch.long))
        lagged_time.append(torch.tensor(u_df["lag_time_cat"].values, dtype=torch.long))
        indices.append(idx)
    ...
    elapsed_time = [torch.cat((torch.zeros(1, dtype=torch.long), i + 1))[:-1] for i in elapsed_time]
    lagged_time = [torch.cat((torch.zeros(1, dtype=torch.long), i + 1))[:-1] for i in lagged_time]
    ...
    # Chunk sequences
    lists = (item_ids, skill_ids, label_inputs, elapsed_time, lagged_time, labels)
...
if __name__ == "__main__":
    ...
    full_df = pd.read_csv(os.path.join('data', args.dataset, 'preparation/preprocessed_data_saint.csv'), sep=",")
    ...
    max_rt = full_df['resp_time_cat'].max() + 2
    max_lt = full_df['lag_time_cat'].max() + 2
